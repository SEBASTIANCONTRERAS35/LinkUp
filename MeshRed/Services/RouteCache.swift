...
class RouteCache {
    ...
    func findRoute(to destination: String) -> RouteInfo? {
        queue.sync {
            ...
            if !route.isValid {
                ...
                print("🗑️ [RouteCache] Expired route to \(destination) removed")
                return nil
            }
        ...
        queue.async(flags: .barrier) { [weak self] in
            ...
            if let existingRoute = self.routes[route.destination] {
                ...
                if shouldUpdate {
                    ...
                    print("📍 [RouteCache] Updated route to \(route.destination): \(route.hopCount) hops via \(route.nextHop)")
                } else {
                    print("⏭️ [RouteCache] Keeping existing better route to \(route.destination)")
                }
            } else {
                // New route
                self.routes[route.destination] = route
                print("📍 [RouteCache] Added route to \(route.destination): \(route.hopCount) hops via \(route.nextHop)")
            }
        }
    }
    ...
    func removeRoute(to destination: String) {
        queue.async(flags: .barrier) { [weak self] in
            if self?.routes.removeValue(forKey: destination) != nil {
                print("🗑️ [RouteCache] Removed route to \(destination)")
            }
        }
    }
    ...
    func removeRoutesVia(nextHop: String) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            for (destination, _) in routesToRemove {
                ...
                print("🗑️ [RouteCache] Removed route to \(destination) (via disconnected peer \(nextHop))")
            }
        }
    }
    ...
    func clearExpiredRoutes() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            if removedCount > 0 {
                print("🗑️ [RouteCache] Cleared \(removedCount) expired routes")
            }
        }
    }
    ...
    func clearAll() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            print("🗑️ [RouteCache] Cleared all \(count) routes")
        }
    }
