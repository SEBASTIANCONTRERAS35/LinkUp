...
class OfflineMapManager: ObservableObject {
    ...
    func downloadCurrentLocation() {
        guard let location = locationService.currentLocation else {
            LoggingService.network.info("⚠️ [OfflineMapManager] No location available")
            return
        }
    ...
    func downloadRegion(center: CLLocationCoordinate2D, name: String, radiusKm: Double = 20.0) {
        LoggingService.network.info("🌍 [OfflineMapManager] Downloading region: \(name)")
    ...
    func enableOfflineMode() {
        ...
        LoggingService.network.info("🔒 [OfflineMapManager] Offline mode enabled")
    }

    /// Enable hybrid mode (cache + network)
    func enableHybridMode() {
        isOfflineModeEnabled = false
        LoggingService.network.info("🌐 [OfflineMapManager] Hybrid mode enabled")
    }
    ...
    func deleteRegion(_ region: DownloadedRegion) {
        ...
        LoggingService.network.info("🗑️ [OfflineMapManager] Deleted region: \(region.name)")
    }
    ...
    func clearAllCache() {
        ...
        LoggingService.network.info("🗑️ [OfflineMapManager] All cache cleared")
    }
    ...
    private func saveDownloadedRegion(name: String, center: CLLocationCoordinate2D, radiusKm: Double, sizeBytes: Int64) {
        ...
        LoggingService.network.info("✅ [OfflineMapManager] Region saved: \(name)")
    }
