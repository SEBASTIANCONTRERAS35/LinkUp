...
class LeaderElection: ObservableObject {
    ...
    private func calculateLocalScore() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            LoggingService.network.info("📊 Leader Score Calculated: \(self.localScore)")
            LoggingService.network.info("   Connections: \(connectionScore), Battery: \(batteryScore)")
            LoggingService.network.info("   Stability: \(stabilityScore), Device: \(deviceScore)")
        }
    }
    ...
    func startElection() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            LoggingService.network.info("🗳️ STARTING ELECTION")
            LoggingService.network.info("   Local ID: \(self.localPeerId)")
            LoggingService.network.info("   Local Score: \(self.localScore)")
            LoggingService.network.info("   Current Term: \(self.currentTerm)")
            LoggingService.network.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    private func declareVictory() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            LoggingService.network.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            LoggingService.network.info("👑 DECLARING VICTORY")
            LoggingService.network.info("   New Leader: \(self.localPeerId)")
            LoggingService.network.info("   Term: \(self.currentTerm)")
            LoggingService.network.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    func handleElectionMessage(_ data: Data, from peerId: String) {
        guard let message = try? JSONDecoder().decode(ElectionMessage.self, from: data) else {
            LoggingService.network.info("❌ Failed to decode election message")
            return
        }
    ...
    private func handleElection(from message: ElectionMessage) {
        LoggingService.network.info("🗳️ Received election message from \(message.senderId) with score \(message.score)")
    ...
    private func handleVictory(from message: ElectionMessage) {
        LoggingService.network.info("👑 Received victory message from \(message.senderId)")
    ...
    private func handleAcknowledgment(from message: ElectionMessage) {
        LoggingService.network.info("✅ Received acknowledgment from \(message.senderId)")
    ...
    private func startElectionTimer() {
        ...
        electionTimer = Timer.scheduledTimer(withTimeInterval: timeout, repeats: false) { [weak self] _ in
            ...
            if !self.currentState.isLeader {
                LoggingService.network.info("⏰ Election timeout - starting election")
                self.startElection()
            }
        }
    }
    ...
    func removeClusterMember(_ peerId: String) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            if peerId == self.currentLeader {
                LoggingService.network.info("👑 Leader disconnected - starting new election")
                self.startElection()
            }
        }
    }
