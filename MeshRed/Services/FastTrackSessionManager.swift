...
class FastTrackSessionManager {
    ...
    private init() {
        print("⚡ FastTrackSessionManager initialized - ALL DELAYS DISABLED")
    }
    ...
    func recordSuccessfulConnection(to peer: MCPeerID) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            if let startTime = self.connectionAttempts[peerKey] {
                ...
                print("⚡ FastTrack: Connected to \(peerKey) in \(String(format: "%.3f", connectionTime))s")

                if connectionTime < 0.5 {
                    print("🏆 ULTRA-FAST: Under 500ms!")
                } else if connectionTime < 1.0 {
                    print("🎯 SUB-SECOND: Under 1s!")
                }
            }
        }
    }
    ...
    func recordDisconnection(from peer: MCPeerID) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            print("⚡ FastTrack: Disconnected from \(peerKey) - INSTANT RECONNECT ALLOWED")
        }
    }
    ...
    func recordConnectionDeclined(to peer: MCPeerID, reason: String) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            print("⚡ FastTrack: Connection declined (\(reason)) - Will retry immediately")
        }
    }
    ...
    func clearCooldown(for peer: MCPeerID) {
        ...
        print("⚡ FastTrack: No cooldowns - \(peer.displayName) ready for instant connection")
    }
    ...
    func resetPeer(_ peer: MCPeerID) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            print("⚡ FastTrack: Instant reset for \(peerKey)")
        }
    }
    ...
    func clearAll() {
        queue.async(flags: .barrier) { [weak self] in
            ...
            print("⚡ FastTrack: Instant clear completed")
        }
    }
