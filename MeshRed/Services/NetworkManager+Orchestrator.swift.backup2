...
extension NetworkManager {
    ...
    var isOrchestratorEnabled: Bool {
        ...
        set {
            ...
            LoggingService.network.info("🎯 Orchestrator \(newValue ? "ENABLED" : "DISABLED")")
        }
    }
    ...
    func setupOrchestratorCallbacks() {
        ...
        orchestrator.leaderElection.onBecomeLeader = { [weak self] in
            ...
            LoggingService.network.info("👑 BECAME CLUSTER LEADER")
            LoggingService.network.info("   Peer: \(self?.localPeerID.displayName ?? "unknown")")
            LoggingService.network.info("   Responsibilities: Connection coordination")
            LoggingService.network.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        ...
        orchestrator.leaderElection.onLoseLeadership = { [weak self] in
            LoggingService.network.info("👤 LOST LEADERSHIP - Now following cluster leader")
        ...
        orchestrator.leaderElection.onLeaderChanged = { [weak self] newLeader in
            LoggingService.network.info("👑 Cluster leader changed to: \(newLeader ?? "none")")
        ...
        NotificationCenter.default.addObserver(
            ...
            LoggingService.network.info("⚡ Connection evicted by pool manager: \(peer.displayName)")
        ...
        NotificationCenter.default.addObserver(
            ...
            LoggingService.network.info("⏰ Idle timeout for \(peer.displayName) after \(Int(idleTime))s")
        ...
        NotificationCenter.default.addObserver(
            ...
            LoggingService.network.info("⚠️ Disconnection predicted for \(peer.displayName) in \(Int(time))s")
        ...
        LoggingService.network.info("✅ Orchestrator callbacks configured")
    }
    ...
    func shouldConnectToDiscoveredPeer(_ peer: MCPeerID, discoveryInfo: [String: String]?) -> Bool {
        ...
        switch decision {
        case .accept:
            LoggingService.network.info("✅ Orchestrator: ACCEPT connection to \(peer.displayName)")
            return true

        case .reject(let reason):
            LoggingService.network.info("❌ Orchestrator: REJECT connection to \(peer.displayName)")
            LoggingService.network.info("   Reason: \(reason)")
            return false

        case .postpone(let until):
            let seconds = until.timeIntervalSinceNow
            LoggingService.network.info("⏰ Orchestrator: POSTPONE connection to \(peer.displayName) for \(Int(seconds))s")
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + max(0, seconds)) { [weak self] in
                ...
                if self.availablePeers.contains(where: { $0.displayName == peer.displayName }) {
                    LoggingService.network.info("🔄 Retrying postponed connection to \(peer.displayName)")
        ...
        case .conditionalAccept(let conditions):
            LoggingService.network.info("✅ Orchestrator: CONDITIONAL ACCEPT for \(peer.displayName)")
            LoggingService.network.info("   Conditions: \(conditions.joined(separator: ", "))")
            return true
        }
    }
    ...
    func shouldAcceptInvitationFromPeer(_ peer: MCPeerID) -> Bool {
        ...
        case .accept, .conditionalAccept:
            LoggingService.network.info("✅ Orchestrator: ACCEPT invitation from \(peer.displayName)")
            return true
        case .reject(let reason):
            LoggingService.network.info("❌ Orchestrator: REJECT invitation from \(peer.displayName)")
            LoggingService.network.info("   Reason: \(reason)")
            return false
        case .postpone:
    ...
    func optimizeConnections() {
        guard isOrchestratorEnabled else {
            LoggingService.network.info("⚠️ Orchestrator disabled - cannot optimize")
            return
        }
    ...
    func forceLeaderElection() {
        guard isOrchestratorEnabled else {
            LoggingService.network.info("⚠️ Orchestrator disabled")
            return
        }
