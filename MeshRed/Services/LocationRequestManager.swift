...
class LocationRequestManager: ObservableObject {
    ...
    func trackRequest(_ request: LocationRequestMessage) {
        ...
        LoggingService.network.info("📍 LocationRequestManager: Tracking request \(request.id) for target \(request.targetId)")
    }
        ...
        if response.responseType == .triangulated,
           ...
           let direction = relativeLocation.targetDirection {
            ...
            if let calculatedLocation = LocationCalculator.calculateTargetLocation(
                ...
                LoggingService.network.info("📍 Calculated target location: \(calculatedLocation.coordinateString)")
            }
        }
    }
    ...
    func shouldRespondToRequest(_ request: LocationRequestMessage) -> Bool {
        ...
        guard autoRespondToRequests else {
            LoggingService.network.info("📍 LocationRequestManager: Auto-respond disabled")
            return false
        }
        ...
        if let allowed = allowedRequesters {
            guard allowed.contains(request.requesterId) else {
                LoggingService.network.info("📍 LocationRequestManager: Requester \(request.requesterId) not in whitelist")
                return false
            }
        }
    ...
    func markRequestTimeout(_ requestId: UUID) {
        DispatchQueue.main.async {
            ...
            if state.retryCount >= self.maxRetries {
                ...
                LoggingService.network.info("❌ LocationRequestManager: Request \(requestId) timed out after \(self.maxRetries) retries")
            } else {
                state.status = .pending
                LoggingService.network.info("🔄 LocationRequestManager: Retrying request \(requestId) (attempt \(state.retryCount + 1)/\(self.maxRetries))")
            }
